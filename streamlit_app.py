...
def read_workbook(xls_bytes: bytes) -> Tuple[List[str], Dict[str, pd.DataFrame]]:
    ...
    for s in sheets:
        ...
        # Ha nincs név szerinti Hint oszlop, próbáljuk a 4. (D) oszlopot
        if hcol is None and len(df.columns) >= 4:
            cand = df.columns[3]
            if cand not in {qcol, acol, lcol}:
                hcol = cand
        ...
        if "Hint" not in df.columns:
            df["Hint"] = ""
        df["Hint"] = df["Hint"].astype(str).fillna("").str.strip()
...
# --------- Segítség expander (mindig látszik) ---------
with st.expander("💡 Segítség"):
    st.write(hint if str(hint).strip() else "_Nincs megadva._")
